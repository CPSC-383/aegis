...
class PredictionHandler:
    ...
    # gid, survivor_id, [agent_id responsible, prediction_correct]
    ...
    _x_test: NDArray[np.float32] = np.load(
        os.path.join(aegis_testing_output_dir, "x_test.npy")
    )
    _y_test: NDArray[np.int64] = np.load(
        os.path.join(aegis_testing_output_dir, "y_test.npy")
    )
