...
from aegis.testing_data.prediction_handler import PredictionHandler
        ...
        try:
            ...
            for name, value_type, _ in options:
                ...
                if option and option.is_set and option.value:
                    if name == "NoKViewer":
                        self._parameters.number_of_agents = int(option.value)
                    elif name == "ProcFile":
                ...
                if command is not None:
                    ...
                    if self._parameters.config_settings is not None:
                        ...
                        if self._parameters.config_settings.handling_messages == ConfigSettings.SEND_MESSAGES_AND_PERFORM_ACTION:
                            print(f"Agent {current_agent.agent_id} sent no action (non-send) command this round.")
                        else:
                            print(f"Agent {current_agent.agent_id} sent no command this round.")
            ...
            if send_message.agent_id_list.is_empty():
                if self._parameters.config_settings is not None:
                    if self._parameters.config_settings.send_messages_to_all_groups:
                        ...
                        self._agent_handler.forward_message_to_group(send_message.get_agent_id().gid, fwd_message)
            else:
            ...
            if self._prediction_handler is not None:
                ...
                if self._prediction_handler.is_group_in_no_pred_yet(agent.agent_id.gid, prediction.surv_id):
                    ...
                    if self._prediction_handler.is_agent_in_saving_group(agent.agent_id, prediction.surv_id):
                        ...
                        self._agent_handler.increase_agent_group_predicted(agent.agent_id.gid, prediction.surv_id, prediction.label, correct_prediction)
