...
if a folder is called `CPSC 383`, the space will cause some undefined behavior when running the system. To avoid this,
rename parent folders to use underscores or hyphens, such as `CPSC_383` or `CPSC-383`.

**Python Version Requirements:** Aegis requires Python version **3.12.x** to function correctly. Ensure your Python version matches this requirement.
...
### Ensure You Are Using Python 3.12.x

Confirm that you are using Python 3.12.x by running the following command:
...
If the version shown is not 3.12.x, you will need to update your Python installation.
