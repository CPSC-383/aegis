import { allGuidesPathfindings } from "content-collections";
...
async function getDocFromParams({ params }: Props) {
  ...
  const entry = allGuidesPathfindings.find((entry) => {
...
export function generateStaticParams(): { slug: string[] }[] {
  return allGuidesPathfindings.map((entry) => ({
...
export default async function Page({ params }: Props) {
  ...
  return (
    <>
      <MobileNavigation sidebarItems={navPathfinding.guidesNav} />

      <div className="flex overflow-hidden gap-8 max-w-full">
        <div className="hidden lg-custom:block">
          <Sidebar items={navPathfinding.guidesNav} />
        </div>
