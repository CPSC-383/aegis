import { allGettingStartedPathfindings } from "content-collections";
...
async function getDocFromParams({ params }: Props) {
  ...
  const entry = allGettingStartedPathfindings.find((entry) => {
...
export function generateStaticParams(): { slug: string[] }[] {
  return allGettingStartedPathfindings.map((entry) => ({
...
export default async function Page({ params }: Props) {
  ...
  return (
    <>
      <MobileNavigation sidebarItems={navPathfinding.gettingStartedNav} />

      <div className="flex overflow-hidden gap-8 max-w-full">
        <div className="hidden lg-custom:block">
          <Sidebar items={navPathfinding.gettingStartedNav} />
        </div>
