import { allGettingStartedMAS } from "content-collections";
...
async function getDocFromParams({ params }: Props) {
  ...
  const entry = allGettingStartedMAS.find((entry) => {
...
export function generateStaticParams(): { slug: string[] }[] {
  return allGettingStartedMAS.map((entry) => ({
...
export default async function Page({ params }: Props) {
  ...
  return (
    <>
      <MobileNavigation sidebarItems={navMas.gettingStartedNav} />

      <div className="flex overflow-hidden gap-8 max-w-full">
        <div className="hidden lg-custom:block">
          <Sidebar items={navMas.gettingStartedNav} />
        </div>
