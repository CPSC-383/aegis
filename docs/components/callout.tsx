  ...
  ({ className, children, title, type = 'info', icon, ...props }, ref) => {
    ...
    return (
      <div
        ...
        {...props}
      >
        <div className={`absolute left-0 top-0 bottom-0 w-1 ${theme.accent} rounded-r-sm`}>
          <div className={`absolute inset-0 ${theme.accent} animate-pulse opacity-60`} />
        </div>

        <div className="absolute top-2 right-2">
          <span className={`text-xs font-mono px-2 py-1 rounded-full ${theme.statusColor} border border-current/30`}>
            {theme.status}
          </span>
        </div>

        <div className="flex-shrink-0 mt-0.5">
          <div className={`p-2 rounded-lg bg-gradient-to-br from-slate-700/50 to-slate-800/50 border border-current/20 ${theme.icon}`}>
            {getIcon()}
          </div>
        </div>
        ...
        <div className="flex flex-col justify-center gap-2 min-w-0 flex-1">
          {title && (
            <div className="flex items-center gap-2">
              <p className={`font-semibold font-mono uppercase tracking-wide text-sm ${theme.title} !my-0`}>
                {title}
              </p>
            </div>
          )}
          <div className={`${theme.content} prose-no-margin empty:hidden leading-relaxed`}>
            {children}
          </div>
        </div>

        <div className={`absolute inset-0 pointer-events-none opacity-5 ${theme.accent} rounded-lg`} />
      </div>
    );
  },
);
