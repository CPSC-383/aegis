...
export default function MobileNavigation({ sidebarItems }: Props) {
  ...
  return (
    <>
      <nav className="lg:hidden w-full flex items-center justify-between px-4 py-3 border-b border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900">
      ...
      <div
        className={`fixed flex flex-col left-0 top-0 z-50 h-full w-80 max-w-[85vw] bg-white dark:bg-zinc-900 shadow-xl transform transition-transform duration-300 ease-out lg:hidden ${isOpen ? "translate-x-0" : "-translate-x-full"
          }`}
      >
        <div className="flex items-center justify-between p-4 border-b border-zinc-200 dark:border-zinc-700">
          <h2 className="font-medium text-zinc-900 dark:text-white">Menu</h2>
          <button
            onClick={closeMenu}
            className="p-2 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors"
