  ...
  return (
    <>
      <Select
        ...
        onValueChange={(value) =>
          setSpecialCellType(value as SpecialCellBrushTypes)
        }
      >
        <SelectTrigger>
          ...
          {Object.values(SpecialCellBrushTypes).map((type) => (
            <SelectItem key={type} value={type}>
              <div className="flex items-center space-x-2">
                {type === SpecialCellBrushTypes.Killer && (
                  <Skull className="w-4 h-4" />
                )}
                {type === SpecialCellBrushTypes.Fire && (
                  <Flame className="w-4 h-4" />
                )}
                {type === SpecialCellBrushTypes.Charging && (
                  <Zap className="w-4 h-4" />
                )}
                {type === SpecialCellBrushTypes.Spawn && (
      ...
      {specialCellType === SpecialCellBrushTypes.Spawn && (
        <>
          <Select
            value={spawnZoneType}
            onValueChange={(value) => setSpawnZoneType(value as SpawnZoneTypes)}
          >
            <SelectTrigger>
                ...
                .map((type) => (
                  <SelectItem key={type} value={type}>
                    <div className="flex items-center space-x-2">
                      ...
                      {type === SpawnZoneTypes.Group && (
                        <User className="w-4 h-4" />
                      )}
                      <span>
                        {type === SpawnZoneTypes.Group ? "Single" : "Any"}
                      </span>
                    </div>
                  </SelectItem>
                ))}
