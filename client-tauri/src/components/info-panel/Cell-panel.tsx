...
function CellPanel({
  ...
  const renderAgentCanvas = useCallback(
    (
  ...
  return (
    <div>
      ...
      <section className="m-2">
        <h3 className="text-lg border-b border-gray-300 pb-2 mb-2">
          Cell Info
        </h3>
        {cellInfo ? (
          <div className="space-y-2">
            <div>
              ...
              <Badge variant="secondary">
                {cellInfo.cell_type.replace(/\w*\./, "")}
              </Badge>
            </div>
            <div>
              Move Cost:{" "}
              <Badge variant="secondary">{cellInfo.stack.move_cost}</Badge>
            </div>
          </div>
        ) : (
