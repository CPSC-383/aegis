...
function RubbleSettings({ rubbleInfo, setRubbleInfo }: Props): JSX.Element {
  const handleEnergyBlur = (e: React.FocusEvent<HTMLInputElement>): void => {
  ...
  const handleAgentsBlur = (e: React.FocusEvent<HTMLInputElement>): void => {
    const value = parseInt(e.target.value)
    if (!isNaN(value)) {
      const newValue = Math.max(0, value)
      setRubbleInfo({
