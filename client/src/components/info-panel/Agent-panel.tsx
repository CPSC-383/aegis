...
function AgentPanel({ selectedAgent, setSelectedAgent, setCellLayers, simulation }: Props) {
    const updateSelectedAgentInfo = () => {
        ...
        const agent = simulation!.getAgentWithIds(selectedAgent.agentId!.id, selectedAgent.agentId!.gid)
        const layers = simulation!.getLayersAtCell(selectedAgent.location!.x, selectedAgent.location!.y)
